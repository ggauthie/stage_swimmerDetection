...
node Split
  in (nbSlice: int param, width: int param, height: int param, src: uchar[3*height*width])
...
node Likelihood
  in (width: int param, height: int param, data: uchar[3*height*width])
  out (likelihood: float[2*height*width])
actor
  preesm(loop_fn="calculate_likelihood",  init_fn="initGMM_Model", incl_file="Code/include/GMM.h", src_file="Code/src/GMM.c")
end;

node MaskCreation
  in (width: int param, height: int param, component_index: int param, likelihood: float[2*height*width])
  out (mask: uchar[height*width])
actor
  preesm(loop_fn="createMaskGMM_model", incl_file="Code/include/GMM.h", src_file="Code/src/GMM.c")
end;

node Merge
  in (nbSlice : int param, width: int param, height: int param, src: uchar[nbSlice*sliceHeight*width])
...
node DrawRectangle
  in (width: int param, height: int param, src: uchar[3*height*width], bb : Rectangle) 
  out (dest: uchar[3*height*width])
actor
...
fun
  ...
  val dest = DrawRectangle width height pixels bb
